...
(function (global, factory) {
    typeof exports === 'object' && typeof module !== 'undefined' ? factory(exports, require('lodash'), require('mithril'), require('mithril/stream'), require('jss'), require('luxon'), require('@sdxmessaging/ui-widgets'), require('pdfjs-dist'), require('js-sha256'), require('gsap'), require('gsap/ScrollToPlugin'), require('pdf-lib'), require('ag-grid-community'), require('pusher-js')) :
    typeof define === 'function' && define.amd ? define(['exports', 'lodash', 'mithril', 'mithril/stream', 'jss', 'luxon', '@sdxmessaging/ui-widgets', 'pdfjs-dist', 'js-sha256', 'gsap', 'gsap/ScrollToPlugin', 'pdf-lib', 'ag-grid-community', 'pusher-js'], factory) :
    (global = typeof globalThis !== 'undefined' ? globalThis : global || self, factory(global.uiBuilder = {}, global._, global.m, global.m.stream, global.jss, global.luxon, global.uiWidgets, global.pdfjsLib, global.sha256, global.gsap, global.ScrollToPlugin, global.PDFLib, global.agGrid, global.Pusher));
})(this, (function (exports, lodash, m, stream, jss, luxon, uiWidgets, PDFJS, jsSha256, gsap$3, ScrollToPlugin, pdfLib, agGridCommunity, Pusher) { 'use strict';
    ...
    var gsap__default = /*#__PURE__*/_interopDefaultLegacy(gsap$3);
    ...
    // Paired with getPercentage for group and overall document field completion %
    function getRequired(fields, viewController) {
        return fields.filter(({ input, data }) => {
            return input && (input.required || viewController.formController.formState.getState(data).required) ? true : false;
        });
    }
    /**
     * Return percentage completion of a given list of overlay fields
     */
    function getPercentage(fields) {
        const completeFields = lodash__default["default"].reduce(fields, (acc, field) => {
            return acc + (field.value && hasValue(field.value()) ? 1 : 0);
        }, 0);
        return (completeFields / fields.length) * 100;
    }
    ...
    class PdfPage {
        oncreate({ dom, attrs: { page, viewport } }) {
            this.renderPage(page, dom, viewport);
        }
        // Only render when scale changes
        onbeforeupdate({ attrs: { viewport: { height } } }, { attrs: { viewport: { height: oldHeight } } }) {
            return height !== oldHeight;
        }
        onupdate({ dom, attrs: { page, viewport } }) {
            this.renderPage(page, dom, viewport);
        }
        view() {
            return m__default["default"]("canvas.w-100.h-100");
        }
        // Ideally cancel rendertask in onbeforeremove hook, but hook isn't called by mithril
        onremove() {
            if (this.renderTask) {
                this.renderTask.cancel();
            }
        }
        renderPage(page, canvas, viewport) {
            const canvasContext = canvas.getContext("2d");
            // Buffer task if rendering is set true
            if (!this.renderTask) {
                ...
                // Handle render completion and rejection (if cancelled)
    ...
    */
    ...
    /**
    * Scale results from scaleSmoothStep to appripriate scaling factors for PDF canvas
    * @param input - Smoothed value (0.25 - 4)
    * @returns Stepped value (0.75, 1.5, 2.25, 3, 4.5, 6)
    */
    function scaleCanvas(input) {
        return Math.ceil(2 * input) * 0.75;
    }
    class PageManager {
        constructor() {
            this.pageBufferSize = 1;
            this.pageBuffer = [];
            this.scrollPctY = 0;
            this.scrollPctX = 0;
            this.visiblePage = stream__default["default"](1);
            this.pageHeight = 1;
            this.pageWidth = 1;
            this.scalar = 1;
            this.leftMargin = 0;
        ...
        oninit({ attrs: { controller } }) {
            // Scale zoom values
            this.smoothZoom = controller.zoom.map(scaleSmoothStep);
            // Clamp canvas scaling on small devices, use DPI on large devices
            this.canvasScale = this.smoothZoom.map((smooth) => smallDevice()
                ? Math.min(1, scaleCanvas(smooth))
                : window.devicePixelRatio * scaleCanvas(smooth));
            this.visiblePage.map(controller.viewPage);
        }
        oncreate({ dom, attrs: { pdf, controller } }) {
            this.container = dom;
            // Populate pageBuffer
            this.getPage(pdf, 1).then((page) => {
                // Store first page for scaling use
                this.firstPage = page;
                this.applyScaling(controller);
            }).then(() => this.scroll(pdf));
            // Handle page resizing
            this.onResize = size.map(() => {
                this.applyScaling(controller);
                // Update relative scroll position (minor delay to help mobile device rotation)
                lodash__default["default"].delay(() => {
                    this.container.scrollTop = this.container.scrollHeight * this.scrollPctY;
                }, 32);
            });
            // Rate limit page scroll event handling
            this.scrollHandler = lodash__default["default"].throttle((event) => {
                this.scroll(pdf);
                controller.managerScroll(event);
            }, 200, { leading: false });
            // Handle page zoom changes
            this.onZoom = this.smoothZoom.map(() => {
                this.updateZoom();
                this.scroll(pdf);
            });
            // Support pinch zoom
            const viewer = dom.firstChild;
            this.pinchZoom = new PinchZoom(this.container, viewer, (newScale) => {
                // Apply scale multiplier to current smooth value
                const newZoom = this.smoothZoom() * newScale;
                // "unsmooth" value back to controller stream
                controller.zoom(Math.log2(newZoom));
                m__default["default"].redraw.sync();
            });
            // Handle page jumping
            controller.jumpPage.map((page) => {
                if (page && page > 0) {
                    const viewScale = this.scalar * this.smoothZoom();
                    const scrollTo = (page - 1) * this.pageHeight * viewScale;
                    this.container.scrollTop = scrollTo;
                }
            });
            controller.managerCreate(this.container);
        }
        onremove() {
            this.visiblePage.end(true);
            this.pinchZoom.destroy();
            this.onZoom.end(true);
            this.canvasScale.end(true);
            this.smoothZoom.end(true);
            this.onResize.end(true);
            this.scrollHandler.cancel();
        }
        onupdate() {
            // Update container scrolling to reflect any scaling changes
            this.container.scrollTop = this.container.scrollHeight * this.scrollPctY;
            this.container.scrollLeft = this.container.scrollWidth * this.scrollPctX;
        }
        view({ attrs: { pdf, controller, overlay } }) {
            const viewScale = this.scalar * this.smoothZoom();
            const pageScale = this.canvasScale();
            return m__default["default"](".flex-auto.overflow-x-auto.overflow-y-scroll.bg-gray.safari-bug", {
                onclick: (event) => {
                    controller.managerClick(event);
                },
                onscroll: (event) => {
                    event.redraw = false;
            ...
            // "Document-sized" page container (fast scaled by PinchZoom)
            m__default["default"](".relative", {
                style: {
                    height: `${pdf.numPages * this.pageHeight * viewScale}px`,
                    width: `${this.pageWidth * viewScale}px`,
                    marginLeft: `${this.leftMargin}px`
                }
            }, lodash__default["default"].map(this.pageBuffer, ((page) => {
                // Scaled and positioned "active" page subset
                const { width, height } = page.getViewport({ scale: 1 });
                const yPos = (page.pageNumber - 1) * this.pageHeight * viewScale;
                return m__default["default"](".absolute.bg-near-white.shadow-5", {
                    key: page.pageNumber,
                    style: {
                        width: `${width}px`,
                        height: `${height}px`,
                        transform: `translateY(${yPos}px) scale(${viewScale})`,
                        transformOrigin: "top left"
                    }
                }, [
                    m__default["default"](overlay, {
                        controller,
                    ...
                    m__default["default"](PdfPage, {
                        page,
                        // Page canvas will redraw if viewport changes
        ...
        applyScaling(controller) {
            if (this.firstPage) {
                const { width, height } = this.firstPage.getViewport({ scale: 1 });
                switch (controller.pageScale) {
                    case 0 /* PageScale.FitWidth */:
                        this.scalar = this.container.clientWidth / width;
                        break;
                    case 1 /* PageScale.FitHeight */:
                        this.scalar = this.container.clientHeight / height;
                        break;
                    // PageScale.Pct100
                    default:
                        this.scalar = 1.5;
                }
                this.pageHeight = height + PageManager.margin;
                this.pageWidth = width;
                this.updateZoom();
            }
        }
        updateZoom() {
            const scalar = this.scalar * this.smoothZoom();
            // Scale pageBufferSize based on container height and an extra page or two
            const bufferExcess = smallDevice() ? 1 : 2;
            const scaleHeight = this.pageHeight * scalar;
            this.pageBufferSize = Math.ceil(this.container.clientHeight / scaleHeight) + bufferExcess;
            const scaleWidth = this.pageWidth * scalar;
            this.leftMargin = this.container.clientWidth > scaleWidth
                ? 0.5 * (this.container.clientWidth - scaleWidth)
                : 0;
        }
        ...
        scroll(pdf) {
            ...
            this.scrollPctY = (this.container.scrollTop / this.container.scrollHeight) || 0;
            this.scrollPctX = (this.container.scrollLeft / this.container.scrollWidth) || 0;
            // Page "mostly" in view
            this.visiblePage(1 + Math.max(0, Math.floor(this.scrollPctY * pdf.numPages + 0.3)));
            // Clamp pageBufferSize to document size
            const bufferSize = Math.min(this.pageBufferSize, pdf.numPages);
            const start = 1 + Math.min(
            // Get buffer "start page" from scroll % and 30% page bias
            Math.max(0, Math.floor(this.scrollPctY * pdf.numPages - 0.3)), 
            // Clamp start page at end of document
            pdf.numPages - bufferSize);
            // Load pages from pdf or reuse from pageBuffer
            let shouldRedraw = false;
            Promise.all(lodash__default["default"].map(new Array(bufferSize), (_, idx) => {
                const num = start + idx;
                const existing = pickByProperty(this.pageBuffer, { pageNumber: num });
                if (existing) {
                    return Promise.resolve(existing);
                }
                else {
                    shouldRedraw = true;
                    return this.getPage(pdf, num);
                }
            })).then((pageList) => {
                this.pageBuffer = pageList;
                if (shouldRedraw) {
        ...
        getPage(pdf, num) {
            return new Promise((resolve) => pdf.getPage(num).then(resolve));
        }
    }
    ...
    function getRareIndex(colorIndex) {
        const lastIdx = 5;
        // indexCount: Record<TuserIndex - 1, number>
        const indexCount = lodash__default["default"].fill(Array(lastIdx), 0);
        // Sum index usage
        colorIndex.forEach((userIndex) => indexCount[userIndex - 1]++);
        // Find least used index
        let rare = 0;
        // Start with first index as "rarest"
        let rareCount = indexCount.shift();
        lodash__default["default"].each(indexCount, (value, index) => {
            if (value < rareCount) {
                // +1 to compensate for array shift
                rare = index + 1;
                rareCount = value;
            }
        });
        // +1 to shift from array index to TUserIndex
        return rare + 1;
    }
    function getUserColor(index) {
        if (!index) {
            return "ui-builder-accent";
        }
        const key = `user${index}`;
        return `ui-builder-${key}`;
    }
    ...
    function isSelectionInput(field) {
        var _a;
        return Boolean(((_a = field.input) === null || _a === void 0 ? void 0 : _a.type) && selectionSet.has(field.input.type));
    }
    function signatureHelper(field, { x1, x2, y1, y2 }) {
        return lodash__default["default"].extend({}, field, {
            // Calculate rectangle height as percentage of width
            heightPct: 100 * (y2 - y1) / (x2 - x1)
        });
    }
    // Build a form using an overlay list
    function buildOverlayFields(overlayList) {
        const overlayForm = lodash__default["default"].map(overlayList, ({ _id, field, initialValue, fileValue, computed, page, rect, preText, postText, user, group, focusOnView = false, fieldSet, data }) => {
            return {
                key: _id,
                input: field.type === "sign" /* FieldType.sign */
                    ? signatureHelper(field, rect)
                    : field,
                fileValue,
                initialValue,
                computed,
                data: Object.assign({ 
                    // form overlay
                    page, rect, preText, postText, focusOnView }, data),
                user,
                group,
                fieldSet,
            };
        });
        return new FormBuilder(overlayForm);
    }
    function createSignatureFile(fileName, text, heightPct) {
        return uiWidgets.dataUrlToFile(uiWidgets.createStamp(text, heightPct), `${fileName}.png`, { text, heightPct });
    }

    class UserTyping {
        view({ attrs: { dotStyle, containerStyle }, children }) {
            return m__default["default"](".flex.items-center.user-typing-dots", { style: containerStyle }, m__default["default"](".user-typing-dot", { style: dotStyle }), m__default["default"](".user-typing-dot", { style: dotStyle }), m__default["default"](".user-typing-dot", { style: dotStyle }), children);
        }
    }

    function getUserOverlay(index) {
        const key = `uiPdfFieldOverlayUser${index}`;
        return theme()[key];
    }
    class PageOverlay {
        oninit({ attrs: { controller, pageNumber } }) {
            this.overlayList = controller.formController.pageFields(pageNumber);
        }
        oncreate() {
            // needed for now until ui-widget invalidInputWrapper gets reviewed
            m__default["default"].redraw();
        }
        // Transform overlay elements from pdf co-ordinates ([0, 0] at bottom-left)
        // To DOM positions ([0, 0] at top-right)
        view({ attrs: { height, controller } }) {
            const activeField = controller.formController.activeField();
            const highlightUser = controller.highlightUser;
            const { colorIndexes, activeFieldUsers, currentUser } = controller.syncController;
            return m__default["default"](".relative.doc-overlay", lodash__default["default"].map(this.overlayList, (field) => {
                var _a;
                if (!field.data || !field.input) {
                    return null;
                }
                const { input, data: { rect, fontSize }, user } = field;
                const pos = rectToHtml(rect, height);
                const formState = controller.formController.formState;
                const state = formState.getState(field.data);
                const stateAttrs = lodash__default["default"](state).values().compact().isEmpty() ? field.input : state;
                const interactive = canEdit(stateAttrs);
                const activeFieldKey = activeFieldUsers.get(field.key);
                // Get overlay user index, prioritise activeUserFields over activeField
                const userIndex = activeFieldKey
                    ? colorIndexes.get(activeFieldKey)
                    : undefined;
                const isHighlighted = highlightUser && highlightUser === ((_a = field === null || field === void 0 ? void 0 : field.user) === null || _a === void 0 ? void 0 : _a.licensee);
                const highlightIndex = isHighlighted && highlightUser ? colorIndexes.get(highlightUser) : undefined;
                return m__default["default"]('.absolute', {
                    style: {
                        ...
                        fontSize: fontSize ? fontSize : `${pos.height * (input.type === "textarea" /* FieldType.textarea */ ? 0.15 : 0.52)}px`,
                        outline: isHighlighted ? `solid 1px` : '',
                        outlineColor: getUserColor(highlightIndex),
                    ...
                    m__default["default"](".div", {
                        ...
                        class: joinClasses([
                            field.key === (activeField === null || activeField === void 0 ? void 0 : activeField.key)
                                ? theme().uiPdfActiveFieldOverlay
                                : interactive ? theme().uiPdfFieldOverlay : "",
                            // User overlay (if field is indexed)
                            userIndex ? getUserOverlay(userIndex) : undefined,
                            interactive ? theme().uiPdfFieldOverlay : "pointer-events-none"
                        ]),
                    }, [
                        // Use container font size
                        user ? m__default["default"]("span.absolute.f7-ns.bottom-0.right-0", {
                            style: {
                                color: getUserColor(userIndex)
                            }
                        }, user.name) : null,
                        activeFieldKey && currentUser.id !== activeFieldUsers.get(field.key)
                            ? m__default["default"](UserTyping, {
                                dotStyle: {
                                    background: getUserColor(userIndex)
                                },
                                containerStyle: {
                                    position: "absolute",
                                    alignItems: "center"
                            ...
                            : null,
                        assembleFormField(field, lodash__default["default"].merge({
                            label: "",
                            readonly: !isSelectionInput(field) ? true : undefined,
                            uiClass: {
                                wrapper: "h-100 w-100",
                                inputWrapper: "pa0 ma0 bn h-100 w-100",
                                input: "h-100",
                                invalidInputWrapper: theme().uiPdfFieldInvalidOverlay,
                                merge: false
                            },
                            tabindex: stateAttrs.disabled ? -1 : undefined
                        }, { required: stateAttrs.required }))
                    ])
                ]);
            }));
        }
    ...
    class FormController {
        constructor(overlays, submitUrl, formStateConfig = []) {
            this.overlays = overlays;
            this.submitUrl = submitUrl;
            this.formStateConfig = formStateConfig;
            this.actionCount = stream__default["default"]();
            this.activeField = stream__default["default"]();
            // Document attachments
            this.attachments = stream__default["default"]([]);
            this.requestParty = "NOT_SET";
            // Active field detail helpers
            this.activeFieldIdx = this.activeField.map((field) => field && field.input
                ? lodash__default["default"].findIndex(this.editList, lodash__default["default"].matches({ input: { id: field.input.id } }))
                : -1);
            this.activeGroup = this.activeField.map((field) => (field === null || field === void 0 ? void 0 : field.group) || "");
            this.activeGroupCount = this.activeGroup.map((groupKey) => {
                if (groupKey in this.fieldGroups) {
                    return this.fieldGroups[groupKey].length;
                }
                else {
                    return -1;
                }
            });
            this.activeGroupIdx = this.activeField.map((field) => {
                if (field && field.input && field.group) {
                    return lodash__default["default"].findIndex(this.fieldGroups[field.group], lodash__default["default"].matches({ input: { id: field.input.id } }));
                }
                else {
                    return -1;
                }
            });
            // List unknown secondary parties in form fields
            this.requestParties = this.overlays.map((list) => lodash__default["default"](list)
                .map((overlay) => { var _a; return (_a = overlay.user) === null || _a === void 0 ? void 0 : _a.requestParty; })
                .uniq()
                .compact()
                .value());
            // Parties requested by primary user
            this.invitedParties = this.overlays.map((list) => lodash__default["default"](list)
                .map((overlay) => { var _a; return (_a = overlay.user) === null || _a === void 0 ? void 0 : _a.invitedParty; })
                .uniq()
                .compact()
                .value());
            // fieldList updated with requested parties as set by primary user
            this.partyFieldList = stream__default["default"].lift((parties, fields) => {
                return lodash__default["default"].map(fields, (overlay) => {
                    var _a, _b;
                    const _overlay = Object.assign({}, overlay);
                    if (((_a = _overlay.user) === null || _a === void 0 ? void 0 : _a.requestParty)
                        && parties.includes((_b = _overlay.user) === null || _b === void 0 ? void 0 : _b.requestParty)) {
                        _overlay.user.invitedParty = this.requestParty;
                        delete _overlay.user.requestParty;
                    }
                    return _overlay;
                });
            }, this.invitedParties, this.overlays);
            this.overlayForm = buildOverlayFields(overlays());
            this.formState = new FormState(this.overlayForm, this.formStateConfig);
            // Limit list to unique widgets that can be modified
            const ownLicensee = profile().licensee;
            this.list = lodash__default["default"](this.overlayForm.fields)
                // Remove fields that are scoped to a different licensee or have a request licensee
                .filter(({ user }) => Boolean(!user || (!(user === null || user === void 0 ? void 0 : user.requestParty) && !(user === null || user === void 0 ? void 0 : user.licensee)) || (user === null || user === void 0 ? void 0 : user.licensee) === ownLicensee))
                .uniqBy(({ input }) => input === null || input === void 0 ? void 0 : input.id)
                .value();
            this.filterEditList();
            // Completed signatures will be removed from _editList, retain "single sign" flag here
            this.isSingleSignForm = this._editList.length === 1 ? isSignField(this.editList[0]) : false;
        }
        get editList() {
        ...
        filterEditList() {
            // TODO re-apply filter when field values change
            // Can be modified
            this._editList = lodash__default["default"].filter(this.list, ({ data, input }) => {
                const newInput = lodash__default["default"].merge({}, this.formState.getState(data), {
                    required: input === null || input === void 0 ? void 0 : input.required,
                    disabled: input === null || input === void 0 ? void 0 : input.disabled,
                    readonly: input === null || input === void 0 ? void 0 : input.readonly
                });
                return canEdit(newInput);
            });
            this._fieldGroups = lodash__default["default"].groupBy(this.editList, (field) => {
                return field.group ? field.group : "ungrouped";
            });
            // Display count of unpopulated required overlay fields
            this.updateActionCount();
        }
        updateActionCount() {
            this.actionCount(lodash__default["default"].sumBy(this.editList, ({ input, value, data }) => {
                if (input && (input.required || this.formState.getState(data).required) && value) {
        ...
        removeFromList(key) {
            removeByProperty(this.list, { key });
            this.filterEditList();
        }
        toFieldsJson() {
            return lodash__default["default"].reduce(this.overlayForm.fields, (acc, field) => {
                const { key, type, value, files } = field;
                if (type === 1 /* WidgetType.Prop */ && value && (value() != null)) {
                    acc[key] = String(value());
                }
                else if (type === 2 /* WidgetType.File */ && files) {
                    const firstFile = lodash__default["default"].head(files());
                    if (firstFile && firstFile.file) {
                        acc[key] = String(firstFile.dataUrl);
                        acc[`${key}-ts`] = new Date(firstFile.file.lastModified).toISOString();
                    }
                }
                return acc;
            }, {});
        ...
        toFormData() {
            ...
            lodash__default["default"].forEach(this.overlayForm.fields, (field) => {
                const { key, type, value, files } = field;
                if (type === 1 /* WidgetType.Prop */ && value && (value() != null)) {
                    ...
                    const firstFile = lodash__default["default"].head(files());
                    if (firstFile && firstFile.file) {
                        formData.append(key, String(firstFile.dataUrl));
                        formData.append(`${key}-ts`, new Date(firstFile.file.lastModified).toISOString());
                    }
                }
            });
            return formData;
        ...
        getWidgetById(id) {
            return lodash__default["default"].find(this.editList, lodash__default["default"].matches({ input: { id } }));
        }
        pageFields(pageNum) {
            return lodash__default["default"].filter(this.overlayForm.fields, (field) => Boolean(field.data && field.data.page === pageNum));
        }
        serialise() {
            return lodash__default["default"].reduce(this.overlayForm.fields, (prev, field) => {
                const { key, value, files } = field;
                if (isPropField(field) && value && (value() != null)) {
                    prev[key] = value();
                }
                else if (isSignField(field) && files) {
                    const file = files()[0];
                    if (file && file.metadata) {
                        // Send signature
                        prev[key] = {
                            text: String(file.metadata.text),
                            heightPct: lodash__default["default"].isNumber(file.metadata.heightPct)
                                ? file.metadata.heightPct
                                : 25
                        };
                    }
                }
        ...
        deserialise(data) {
            lodash__default["default"].forEach(data, (value, key) => {
                if (typeof value === "object") {
                    // Assemble file from IFileState and this.patchFile
                    this.patchFile(key, createSignatureFile(key, value.text, value.heightPct));
        ...
        patchValue(key, value) {
            if (key in this.overlayForm.streamMap) {
                this.overlayForm.streamMap[key].value(value);
            }
        }
        patchFile(key, value) {
            if (key in this.overlayForm.streamMap) {
                const formStream = this.overlayForm.streamMap[key];
                if (formStream.type === 2 /* WidgetType.File */) {
                    formStream.files([value]);
                }
            }
        }
        submitForm({ _id }) {
            const formData = this.toFormData();
            formData.append("item_id", _id);
            // formData.append("file_id", /* Pdf viewer file _id */);
            formData.append("fields_json", JSON.stringify(this.toFieldsJson()));
            return sdxRequest({
                method: "POST",
                url: this.submitUrl,
                headers: getDocumentRequestAuthHeaders(),
                body: formData
            });
        }
        updateRequestParties(id) {
            const index = this.requestParties().indexOf(id);
            if (index !== -1) {
                const value = this.requestParties()[index];
                this.requestParty = value;
                // Remove the party from the requestParties
                streamArrayPullAt(this.requestParties, [index]);
                // Add/replace the party into the requestedParties
                streamArrayPush(this.invitedParties, value);
            }
        }
        addLicenseeToFieldList(id, licensee) {
            const updated = this.partyFieldList().map((field) => {
                const { user } = field;
                if ((user === null || user === void 0 ? void 0 : user.invitedParty) === id) {
                    return Object.assign(Object.assign({}, field), { user: Object.assign(Object.assign({}, user), { licensee }) });
                }
                return field;
            });
            this.partyFieldList(updated);
        }
    }
    ...
    function syncEvent(licensee) {
        return `${"client-sync" /* FieldEvents.Sync */}-${licensee}`;
    }
    class SyncController {
        constructor(channelName, formAction, formController) {
            this.channelName = channelName;
            this.formAction = formAction;
            this.formController = formController;
            this._multiUser = false;
            this._saveSupported = false;
            this._isDirty = false;
            this.currentUser = {
                id: profile().licensee,
                info: profile()
            ...
            //User Colors
            this.colorIndexes = new Map();
            //Active users
            this.activeUsers = [];
            // user id, field key
            this.activeUsersFields = new Map();
            // Inverse of activeUsersFields
            this.activeFieldUsers = new Map();
            this.activeFieldSet = this.formController.activeField.map((field) => {
                var _a;
                if (!field)
                    return [];
                if ((_a = field.input) === null || _a === void 0 ? void 0 : _a.name) {
                    return lodash__default["default"].filter(this.formController.editList, (e) => {
                        var _a, _b;
                        return ((_a = e.input) === null || _a === void 0 ? void 0 : _a.name) === ((_b = field.input) === null || _b === void 0 ? void 0 : _b.name);
                    });
                }
                else if (field.fieldSet) {
                    return filterByProperty(this.formController.editList, {
            ...
            this._saveSupported = Boolean(formAction.saveUrl && formAction.saveUrl.length);
            this.saveStateVisibility = () => {
                if (document.visibilityState === 'hidden') {
                    this.saveState();
                }
            };
            window.addEventListener('visibilitychange', this.saveStateVisibility);
            // Subscribe to form field events
            if (pusher().presenceEnabled) {
                this.channel = pusher().pusher.subscribe(this.channelName);
                this.channel.bind("pusher:subscription_succeeded", lodash__default["default"].bind(this.onSubscribe, this));
                this.channel.bind("pusher:member_added", lodash__default["default"].bind(this.addMember, this));
                this.channel.bind("pusher:member_removed", lodash__default["default"].bind(this.removeMember, this));
                // Handle sync event from "Lead" user (if appropriate)
                this.channel.bind(syncEvent(this.currentUser.id), lodash__default["default"].bind(this.syncState, this));
            }
            else {
                this.loadState();
            }
        }
        get mutltiUser() { return this._multiUser; }
        get saveSupported() { return this._saveSupported; }
        get syncSupported() { return this._saveSupported || this._multiUser; }
        get isDirty() { return this._isDirty; }
        getUserColorFromId(licensee) {
            const userIndex = this.colorIndexes.get(licensee);
            return getUserColor(userIndex);
        }
        broadcastFieldChange(field) {
            const fieldKey = field ? field.key : null;
            const occupied = fieldKey ? this.activeFieldUsers.has(fieldKey) : false;
            // Cancel event when selecting own field
            if (fieldKey && occupied && this.activeFieldUsers.get(fieldKey) === this.currentUser.id) {
                return;
            }
            if (this.saveDocumentState) {
                this.saveDocumentState();
            }
            // Prepare change message, set selected field
            const former = this.formController.activeField();
            const message = {
                select: occupied ? null : fieldKey,
                change: former && this.activeFieldUsers.get(former.key) === this.currentUser.id ?
                    lodash__default["default"](this.activeFieldSet())
                        .map(this.createChange)
                        .compact()
                        .value()
                    : []
            };
            // Determine if previous field should be sent
            this._isDirty = message.change.length > 0;
            // Reflect field selection locally
            this.updateActiveUserField(this.currentUser.id, message.select);
            // Broadcast selection and/or change to other users
            if (this.isConnected(this.channel) && this._multiUser) {
                this.channel.trigger("client-change" /* FieldEvents.Change */, message);
            }
        }
        createChange(field) {
            if (isPropField(field) && field.value) {
                return {
                    key: field.key,
                    value: field.value()
                };
            }
            else if (isSignField(field) && field.files) {
        ...
        loadState() {
            if (this.formAction.loadUrl) {
                sdxRequest(this.formAction.loadUrl)
                    .then((res) => {
                    this.formController.deserialise(res);
                })
                    .catch(({ code }) => {
                    if (code === 404) {
                        dialog({
                            ...
                            context: "error" /* ColorContext.error */,
                            duration: 5000,
                            position: exports.DialogPosition.bottomRight
                        });
                    }
                    else {
                        dialog({
        ...
        saveState() {
            return __awaiter$4(this, void 0, void 0, function* () {
                if (this._saveSupported && this.isLeader()) {
                    const path = this.formAction.saveUrl;
                    if (!path)
                        return;
                    this.saving(true);
                    ...
                    try {
                        yield sdxRequest({
                            url: path,
                            method: "POST",
                            body: this.formController.serialise(),
                        });
                        this._isDirty = false;
                        const timeDelta = new Date().valueOf() - timeStart;
                        this.redrawSyncButton(timeDelta);
                    }
                    catch (error) {
                        dialog({
                            type: exports.DialogType.notification,
                ...
                // Pretend to save for non leaders
                else {
                    this._isDirty = false;
                    this.saving(true);
                    this.redrawSyncButton(500);
                }
            });
        }
        redrawSyncButton(timeDelta) {
            const forcedTime = 2000;
            const timeOut = Math.max(forcedTime - timeDelta, 0);
            setTimeout(() => {
                this.saving(false);
                m__default["default"].redraw();
            }, timeOut);
        }
        onSubscribe({ count, me }) {
            if (count === 1) {
                this.addMember(me);
                this.listenForChange();
                this.loadState();
            }
            // Await FieldEvents.Sync from "Lead" user
        }
        // Handle field change events
        listenForChange() {
            if (this.channel) {
                this.channel.bind("client-change" /* FieldEvents.Change */, lodash__default["default"].bind(this.onUserChange, this));
            }
        }
        isConnected(channel = this.channel) {
            return channel ? channel.subscribed : false;
        }
        isLeader() {
            if (this.activeUsers.length) {
                return this.activeUsers[0].licensee === this.currentUser.id;
            }
            return true;
        }
        syncState({ form, users, activeUsersFields, colorIndexes }) {
            this.formController.deserialise(form);
            this.activeUsers = users;
            this.updateMultiUser();
            setMap(this.activeUsersFields, activeUsersFields);
            this.indexActiveFields();
            setMap(this.colorIndexes, colorIndexes);
            this.listenForChange();
            m__default["default"].redraw();
        }
        addMember({ info }) {
            this.activeUsers.push(info);
            this.updateMultiUser();
            // Assign the least used color to new user
            const colorIndex = getRareIndex(this.colorIndexes);
            this.colorIndexes.set(info.licensee, colorIndex);
            if (this.isConnected(this.channel) && this.isLeader() && this._multiUser) {
                const message = {
                    form: this.formController.serialise(),
                    users: this.activeUsers,
                    colorIndexes: mapToObject(this.colorIndexes),
                    activeUsersFields: mapToObject(this.activeUsersFields)
                };
                this.channel.trigger(syncEvent(info.licensee), message);
            }
            m__default["default"].redraw();
        }
        removeMember(member) {
            removeByProperty(this.activeUsers, { licensee: member.id });
            this.updateMultiUser();
            this.activeUsersFields.delete(member.id);
            this.colorIndexes.delete(member.id);
            this.indexActiveFields();
            m__default["default"].redraw();
        }
        updateMultiUser() {
            this._multiUser = this.activeUsers.length > 1;
        }
        updateActiveUserField(userId, fieldKey) {
            this.activeUsersFields.set(userId, fieldKey);
            this.indexActiveFields();
        }
        indexActiveFields() {
            //Re-index fields
            this.activeFieldUsers.clear();
            this.activeUsersFields.forEach((fieldKey, userId) => {
                if (fieldKey) {
                    this.activeFieldUsers.set(fieldKey, userId);
                }
            });
        }
        onUserChange(event, meta) {
            if (event.change.length > 0 && this.saveSupported) {
                this._isDirty = true;
            }
            lodash__default["default"].each(event.change, (field) => {
                if (field) {
                    if ("value" in field) {
                        this.formController.patchValue(field.key, field.value);
                    }
                    else {
                        const { text, heightPct } = field;
                        this.formController.patchFile(field.key, createSignatureFile(field.key, text, heightPct));
                        this.formController.removeFromList(field.key);
                    }
                }
            });
            this.formController.filterEditList();
            if (this.saveDocumentState) {
                this.saveDocumentState();
            }
            this.updateActiveUserField(meta.user_id, event.select);
            m__default["default"].redraw();
        }
        destroy() {
            if (this.channel) {
                pusher().pusher.unsubscribe(this.channelName);
            }
            if (this.saveDocumentState) {
                this.saveDocumentState.cancel();
            }
            this.saveState();
            window.removeEventListener('visibilitychange', this.saveStateVisibility);
        }
    }
    ...
    class ViewController {
        constructor(item, overlayList, formAction, documentUrl, onSubmit, onClose, navigateOnClose, metadata, customAction, useItemOverlays) {
            this.item = item;
            this.formAction = formAction;
            ...
            this.viewState = stream__default["default"]("view" /* PdfState.view */);
            this.showRequestPartysFab = false;
            // focusInput & autoSubmit
            this.focusInput = false;
            // User highlight
            this.highlightTimeout = null;
            this._highlightUser = null;
            // Page manager controller
            this.pageScale = smallDevice() ? 0 /* PageScale.FitWidth */ : 2 /* PageScale.Pct100 */;
            this.zoom = stream__default["default"](0 /* ZoomAmount.Default */);
            this.jumpPage = stream__default["default"]();
            this.viewPage = stream__default["default"]();
            this.jumpDelay = null;
            //Document submission
            this.submitted = stream__default["default"](false);
            this.tabControls = (event) => {
                var _a;
                if (event.key === 'Tab') {
                    if (!((this.lastActive() && !event.shiftKey) || (this.firstActive() && event.shiftKey))) {
                        event.preventDefault();
                        // Help force stream update value
                        (_a = document.getElementById("overlay-assist")) === null || _a === void 0 ? void 0 : _a.blur();
                        const direction = event.shiftKey ? -1 : 1;
                        this.setNextWidget(direction);
                        m__default["default"].redraw();
                    }
                }
            };
            // Initialize the form controller
            const itemOverlays = filterByProperty(this.item.actions, { type: "overlay" /* ActionType.overlay */ });
            // const mergeKey: keyof IOverlay = "_id";
            const mergeKey = "field.id";
            const formControllerOverlays = useItemOverlays ? itemOverlays : lodash__default["default"].unionBy(itemOverlays, overlayList, lodash__default["default"].property(mergeKey));
            // Override given form action if present on item
            const foundAction = pickByProperty(this.item.actions, { type: "form" /* ActionType.form */ });
            const hasAction = Boolean(foundAction || this.formAction._id);
            if (hasAction) {
                this.formAction = foundAction || this.formAction;
                this.initialPdfState = this.formAction.initialPdfState;
            }
            this.formController = new FormController(stream__default["default"](formControllerOverlays), this.formAction.url, this.formAction.formState);
            // Fab controllers
            this.showRequestPartysFab = Boolean(profile().role === "primary" && (this.formController.requestParties().length || this.formController.invitedParties().length));
            // Mark form as complete if there is no next empty widget
            this.showSubmitFab = stream__default["default"].lift((newState, oustandingFields) => {
                return newState === "edit" /* PdfState.edit */ && oustandingFields === 0;
            }, this.viewState, this.formController.actionCount);
            // If only a single sign field automatically submit the form
            //THIS CANNOT BE REACHED
            this.showSubmitFab.map((show) => {
                if (show && this.formController.isSingleSignForm) {
                    if (!this.submitted()) {
                        this.submit();
                    }
                }
            });
            const hash = jsSha256.sha256(`${this.item._id}-${this.documentUrl.slice(-20)}`);
            this.syncController = new SyncController(`presence-${hash}`, this.formAction, this.formController);
            if (hasAction) {
                this.setDefaultState();
            }
            if (!foundAction && this.customAction) {
                this.viewState("custom" /* PdfState.custom */);
            }
            // Store key of any overlay that should auto focus when scrolling into view
            const focusOverlay = this.formController.isSingleSignForm
                ? this.formController.editList[0]
                : lodash__default["default"].find(this.formController.editList, lodash__default["default"].matches({ data: { focusOnView: true } }));
            this.focusOnViewId = focusOverlay === null || focusOverlay === void 0 ? void 0 : focusOverlay.key;
        }
        get highlightUser() {
            return this._highlightUser;
        }
        submit() {
            if (this.onSubmit)
                this.onSubmit();
            this.submitted(true);
            this.formController.submitForm(this.item)
                .then(() => {
                this.viewState("complete" /* PdfState.complete */);
                this.setActive();
            })
                .catch(() => {
                this.setActive();
                dialog({
                    ...
                    context: "error" /* ColorContext.error */,
                    message: 'Unable to send form, would you like to try again?',
                    ...
                    cancelButton: {
                        onclick: () => {
                            ...
                            if (this.formController.isSingleSignForm) {
                                this.resetInitialStateIfPossible("call" /* PdfState.call */);
                            }
                            else {
                                this.viewState("edit" /* PdfState.edit */);
                            }
                        }
                    },
                });
        ...
        resetInitialStateIfPossible(altState = "call" /* PdfState.call */) {
            this.viewState(this.initialPdfState ? this.initialPdfState : altState);
        }
        setDefaultState() {
            this.resetInitialStateIfPossible("call" /* PdfState.call */);
            if (this.initialPdfState === "edit" /* PdfState.edit */) {
                if (this.lastFocusedField) {
                    this.setActive(this.lastFocusedField);
                }
                else {
                    this.setNextEmptyWidget();
                }
            }
        }
        firstActive() {
            const activeFieldSet = this.syncController.activeFieldSet();
            const edge = lodash__default["default"].head(activeFieldSet);
            return lodash__default["default"].findIndex(this.formController.editList, edge) === 0;
        }
        lastActive() {
            const activeFieldSet = this.syncController.activeFieldSet();
            const edge = lodash__default["default"].last(activeFieldSet);
            return lodash__default["default"].findIndex(this.formController.editList, edge) === this.formController.editList.length - 1;
        }
        setActive(field) {
            var _a;
            this.syncController.broadcastFieldChange(field);
            this.formController.activeField(field);
            if (field) {
                this.viewState("edit" /* PdfState.edit */);
                // Indicate that PdfFooter should focus on the input after redraw
                if (!isSelectionInput(field)) {
                    this.focusInput = true;
                    // Focus hidden input on prop widgets (prevent iOS keyboard dismiss)
                    if (isPropField(field)) {
                        (_a = document.getElementById("hidden-input")) === null || _a === void 0 ? void 0 : _a.click();
                    }
                }
        ...
        setGroup(groupId) {
            const group = this.formController.fieldGroups[groupId];
            if (group && group.length) {
                this.setActive(group[0]);
            }
        }
        setNextWidget(direction = 1) {
            // Find next/previous widget
            const currIdx = this.formController.activeFieldIdx();
            const activeFieldSet = this.syncController.activeFieldSet();
            if (currIdx !== -1) {
                const edge = direction === 1 ? lodash__default["default"].last(activeFieldSet) : lodash__default["default"].head(activeFieldSet);
                const newIdx = lodash__default["default"].findIndex(this.formController.editList, edge) + direction;
                if (newIdx >= 0) {
                    // non-final editable field
                    if (newIdx < this.formController.editList.length) {
                        this.setActive(this.formController.editList[newIdx]);
                    }
                    else {
                        // deselect & go to submit footer at final field
                        this.setActive();
                    }
                }
            }
            else {
                const isRemoved = this.formController.editList.findIndex((field) => {
                    var _a;
                    return field.key === ((_a = this.lastFocusedField) === null || _a === void 0 ? void 0 : _a.key);
                }) === -1;
                if (direction === -1) {
                    if (!isRemoved) {
                        // shift-tabbing goes back to last focused field
                        this.setActive(this.lastFocusedField);
                    }
                    else if (this.formController.editList.length > 0) {
                        // shift-tabbing goes to the final editable field
                        this.setActive(this.formController.editList[this.formController.editList.length - 1]);
                    }
                }
                else {
                    this.setNextEmptyWidget();
        ...
        // Select first overlay or submit if no user input is required
        startFormAction() {
            if (this.formController.overlayForm.fields.length) {
                if (this.formController.isSingleSignForm && this.lastFocusedField) {
                    this.setActive(this.lastFocusedField);
                }
                else {
                    this.setNextEmptyWidget();
        ...
        setNextEmptyWidget() {
            this.setActive(lodash__default["default"].find(this.formController.editList, ({ value }) => {
                return value ? !hasValue(value()) : true;
            }));
        }
        // Active field set helpers
        getActiveFieldSet() {
            return this.syncController.activeFieldSet;
        }
        getFormActionSets() {
            var _a;
            return (_a = this.syncController.formAction) === null || _a === void 0 ? void 0 : _a.sets;
        }
        formActionSetSelectionCount({ fieldSet }) {
            var _a;
            const actionSets = this.getFormActionSets();
            return fieldSet && actionSets
                ? (_a = pickByProperty(actionSets, { name: fieldSet })) === null || _a === void 0 ? void 0 : _a.selectionCount
                : undefined;
        }
        // Active field helpers
        activeFieldInSet() {
            return this.getActiveFieldSet()().length > 1;
        }
        getActiveField() {
            return this.getActiveFieldSet()()[0];
        }
        loadMetadata() {
            return __awaiter$3(this, void 0, void 0, function* () {
                yield sdxRequest({
                    ...
                    method: "GET",
                    headers: Object.assign({ Accept: "application/json;metadata=custom" }, this.getAuthHeaders())
                })
                    .then((metadata) => {
                    this.metadata = metadata;
                });
        ...
        postRequestParty({ email, mobile, fullName, key }) {
            var _a;
            return __awaiter$3(this, void 0, void 0, function* () {
                if (this.metadata) {
                    const state = this.metadata.documentState[key];
                    if (state) {
                        const { uuid } = state;
                        const request_licensee = `${profile().licensee}-${uuid}`;
                        this.formController.addLicenseeToFieldList(key, request_licensee);
                        this.formController.updateRequestParties(key);
                        return sdxRequest({
                            url: ((_a = application().requestParty) === null || _a === void 0 ? void 0 : _a.path) || "/404",
                            method: "POST",
                            body: {
                                request_party: key,
                                request_licensee,
                                request_fullName: fullName,
                                request_firstName: fullName.split(" ")[0],
                                request_lastName: fullName.split(" ")[1],
                                request_email: email,
                                request_mobile: mobile,
                                request_item: Object.assign(Object.assign({}, this.item), { actions: [
                                        this.formAction,
                                        ...this.formController.partyFieldList()
                                    ], tags: [{ ctx: "root", val: "root" }] })
                            }
                        });
                    }
                }
        ...
        managerCreate(container) {
            this.container = container;
            // Support jumping to the active overlay
            this.formController.activeField.map((field) => {
                var _a;
                ...
                if (field && field.input && ((_a = field.data) === null || _a === void 0 ? void 0 : _a.page)) {
                    const { input: { id }, data: { page } } = field;
                    this.jumpDelay = setTimeout(() => {
                        const input = document.getElementById(id);
                        if (input) {
                            // Focus input
                            const containerRect = this.container.getBoundingClientRect();
                            const inputRect = input.getBoundingClientRect();
                            const offsetY = smallDevice()
                                ? containerRect.height - inputRect.height - 20
                                : containerRect.height * 0.5 - inputRect.height * 0.5;
        ...
        checkTarget(target) {
            // "Basic" inputs have id prop
            if (target.id) {
                return this.formController.getWidgetById(target.id);
            }
            // Radio/Checkbox/File inputs have a label with data-input-id
            const widgetLabel = target.closest("label");
            return widgetLabel
                ? this.formController.getWidgetById(widgetLabel.dataset.inputId)
                : undefined;
        }
        managerClick({ target }) {
            if (target) {
                this.setActive(this.checkTarget(target));
            }
        }
        managerScroll(_) {
            // Only focus if pdf is in "call to action" mode
            if (this.focusOnViewId && this.viewState() === "call" /* PdfState.call */) {
                const target = document.getElementById(this.focusOnViewId);
                const targetY = target ? target.getBoundingClientRect().y : -1;
                if (targetY && targetY > 0 && targetY < this.container.clientHeight) {
                    this.setActive(this.formController.getWidgetById(this.focusOnViewId));
                    // Remove the key so action is only triggered once
                    this.focusOnViewId = undefined;
                    m__default["default"].redraw();
                }
            }
        }
        ...
        adjustZoom(delta) {
            // Clamp zoom value
            this.zoom(Math.max(-2 /* ZoomAmount.Min */, Math.min(this.zoom() + delta, 2 /* ZoomAmount.Max */)));
        }
        destroy() {
            if (this.jumpDelay) {
    ...
    class OverlaySpinner {
        view({ attrs: { active, text, style = {} } }) {
            return m__default["default"](".absolute.flex", {
                class: theme().uiOverlaySpinnerWrapper,
                style: Object.assign({
                    opacity: active ? 0.6 : 0,
                    visibility: active ? "visible" : "hidden",
                }, style)
            }, m__default["default"](".flex.flex-column.items-center", {
                class: theme().uiOverlaySpinnerInnerWrapper
            }, m__default["default"]("i", {
                class: theme().uiOverlaySpinnerIcon
            }), m__default["default"](".ma2", {
                class: theme().uiOverlaySpinnerText
            }, text)));
        }
    }
    ...
    function getInitials({ firstName, lastName }) {
        const firstChar = firstName === null || firstName === void 0 ? void 0 : firstName.charAt(0);
        const secondChar = lastName === null || lastName === void 0 ? void 0 : lastName.charAt(0);
    ...
    class ActiveUsers {
        view({ attrs: { viewController } }) {
            const { currentUser } = viewController.syncController;
            return m__default["default"]("div", {
                class: theme().uiActiveUsersWrapper
            }, lodash__default["default"](viewController.syncController.activeUsers)
                .sortBy((user) => user.licensee === currentUser.id ? 0 : 1)
                .map((user) => {
                const activeUser = user.licensee === currentUser.id;
                const userColor = viewController.syncController.getUserColorFromId(user.licensee);
                return m__default["default"](".pointer", {
                    class: theme().uiActiveUser,
                    style: {
                        borderColor: userColor,
                        background: userColor,
                        boxShadow: !activeUser && 'none',
                        filter: activeUser ? 'brightness(1.05)' : 'brightness(90%)',
                    },
                    onclick: () => viewController.setHighlightUser(user.licensee)
                }, getInitials(user));
            })
                .value());
        }
    ...
    class Fab extends Themable {
        oninit({ attrs: { applyTheme } }) {
            this.createTheme(applyTheme);
        }
        onbeforeupdate({ attrs: { applyTheme } }) {
            this.updateTheme(applyTheme);
        }
        view({ attrs: { onclick, icon, style, active, label = "" } }) {
            return m__default["default"]("button.pointer", {
                style,
                class: joinClasses([
                    icon,
                    this.overwrite.uiFab,
                    active ? theme().uiFabActive : theme().uiFab,
                ]),
                onclick
            }, label);
        }
    ...
    class FabCounter {
        view({ attrs: { count, style = {} } }) {
            const scale = count > 0 ? 1 : 0;
            return m__default["default"]("div", {
                class: theme().uiAnimatedFabCounterWrapper,
                style: Object.assign(Object.assign({}, style), { transform: `scale(${scale})`, transition: "opacity 0.3s ease-in-out, transform 0.3s ease-in" })
            }, count);
        }
    }
    ...
    class FileFabFile extends AnimatedFabItem {
        view({ attrs: { onclick, fileName, style } }) {
            return m__default["default"](".flex.br-pill.items-center", {
                class: joinClasses([
                    theme().uiAnimatedFabItemWrapper,
                    theme().uiFileFabFileWrapper,
                    this.animationClass
                ]),
                style: Object.assign(Object.assign({}, style), this.animationStyle),
            }, [
                m__default["default"]("div.truncate", fileName),
                m__default["default"]("i.fal.fa-times.ph2", {
                    onclick
                })
            ]);
        }
    ...
    const fileWrapperClass = createResponsiveClass({
        key: "FileInput",
        style: {
            visibility: "hidden",
            position: "absolute",
    ...
    const clickClass = createResponsiveClass({
        key: "fabClickClass"
    });
    class FileFab extends Themable {
        constructor() {
            super(...arguments);
            this.showFileList = stream__default["default"](false);
            // evListener is only for click events
            this.evListener = (ev) => {
        ...
        onFabClick(fileList) {
            if (fileList().length === 0 || this.showFileList())
                this.fileUploaderElem.click();
            else
                this.showFileList(!this.showFileList());
        }
        onDelete(fileList, index) {
            fileList([
        ...
        getCount(fileList) {
            return this.showFileList() ? 0 : fileList().length;
        }
        getIcon() {
            return this.showFileList() ? "far fa-plus" : "far fa-paperclip";
        }
        oninit({ attrs: { applyTheme } }) {
            this.createTheme(applyTheme);
        }
        oncreate({ dom }) {
            this.dom = dom;
            const uploadWrapper = dom.querySelector("[class*=FileInput]");
            this.fileUploaderElem = uploadWrapper.querySelector("input");
        ...
        view({ attrs: { style, active, fileList } }) {
            return m__default["default"](".flex.flex-column.items-end.fileFabContainer", [
                fileList().map(({ name }, i) => {
                    return m__default["default"](FileFabFile, {
                        index: i,
                        offset: fileList().length - i,
                        fileName: name,
                        open: this.showFileList,
                        onclick: () => this.onDelete(fileList, i)
                    });
                }),
                m__default["default"](".relative", m__default["default"](FabCounter, {
                    count: this.getCount(fileList)
                }), m__default["default"](Fab, {
                    style,
                    applyTheme: {
                        uiFab: {
                            classes: joinClasses([
                                theme().uiAnimatedFabWrapper,
                                this.overwrite.uiAnimatedFabWrapper,
                                theme().uiFileFabWrapper,
                                this.overwrite.uiFileFabFileWrapper,
                            ])
                        },
                    },
                    icon: this.getIcon(),
                    active,
                    onclick: () => this.onFabClick(fileList)
                })),
                m__default["default"](uiWidgets.FileMulti, {
                    value: fileList,
                    showDisplay: false,
                    field: {
                        id: "input",
                        uiClass: {
                            wrapper: fileWrapperClass,
                            input: clickClass
                        }
                    }
                })
    ...
    class RequestIndividualParty extends AnimatedFabItem {
        view({ attrs: { party, onclick, style = {}, icon } }) {
            return m__default["default"](".flex.br-pill.items-center.pointer", {
                class: joinClasses([
                    theme().uiAnimatedFabItemWrapper,
                    theme().uiRequestIndividualPartyWrapper,
                    this.animationClass
                ]),
                style: Object.assign(Object.assign({}, style), this.animationStyle),
                onclick: () => {
                    var _a, _b, _c;
                    dialog({
                        title: ((_a = application().requestParty) === null || _a === void 0 ? void 0 : _a.title) || 'Invite to Sign',
                        subTitle: (_b = application().requestParty) === null || _b === void 0 ? void 0 : _b.subTitle,
                        bodyText: (_c = application().requestParty) === null || _c === void 0 ? void 0 : _c.bodyText,
                        type: exports.DialogType.invite,
                        submitButton: {
                            label: 'Send Invite',
                            onclick
                        },
                ...
                m__default["default"]("div.truncate", party),
                m__default["default"]("i.ph2", { class: icon || "far fa-plus" })
            ]);
        }
    }

    class RequestPartyFab extends Themable {
        constructor() {
            super(...arguments);
            this.showParties = stream__default["default"](false);
            // evListener is only for click events
            this.evListener = (ev) => {
                if (!this.dom.contains(ev.target))
        ...
        oninit({ attrs: { applyTheme, viewController } }) {
            this.createTheme(applyTheme);
            this.showParties.map(m__default["default"].redraw);
            this.allParties = stream__default["default"].lift((requestParties, invitedParties) => {
                return [...requestParties, ...invitedParties];
            }, viewController.formController.requestParties, viewController.formController.invitedParties);
            this.allParties.map((parties) => this.numParties = parties.length);
        }
        oncreate({ dom }) {
            this.dom = dom;
            window.addEventListener("click", this.evListener);
        ...
        view({ attrs: { style, active, viewController } }) {
            const { requestParties } = viewController.formController;
            return [
                m__default["default"](".flex.flex-column.items-end", [
                    lodash__default["default"].map(this.allParties(), (id, i) => {
                        const notSet = requestParties().includes(id);
                        return m__default["default"](RequestIndividualParty, {
                            style: notSet ? { background: "var(--ui-builder-warn)" } : undefined,
                            index: i,
                            offset: this.numParties - i,
                            party: id,
                            open: this.showParties,
                            icon: notSet ? "far fa-plus" : "far fa-pencil",
                            onclick: ({ fullName, email, mobile }) => {
                                viewController.postRequestParty({
                                    email: email.trim(),
                                    mobile: mobile.trim(),
                    ...
                    m__default["default"](".relative", m__default["default"](FabCounter, {
                        count: this.showParties() ? 0 : viewController.formController.requestParties().length,
                        style: {
                            background: "var(--ui-builder-warn)",
                            color: "var(--ui-builder-text-invert)"
                        }
                        ...
                        applyTheme: {
                            uiFab: {
                                classes: joinClasses([
                                    theme().uiAnimatedFabWrapper,
                                    this.overwrite.uiAnimatedFabWrapper,
                                    theme().uiRequestPartyFabWrapper,
                                    this.overwrite.uiRequestPartyFabWrapper
                                ])
                            },
                        },
                        icon: "far fa-user",
    ...
    class PdfFooterCall {
        oncreate({ attrs: { viewController } }) {
            this.onclick = viewController.startFormAction.bind(viewController);
        }
        view({ attrs: { viewController } }) {
            return [
                m__default["default"](uiWidgets.NavButton, {
                    title: "Start",
                    icon: viewController.syncController.formAction.icon || "fal fa-angle-down mt1",
                    classes: joinClasses([
                        "absolute f2 f1-ns top-0 br-100 shadow-4 pulse flex items-center justify-center",
                        theme().uiPdfStartButton
                    ]),
                    // Activate next unset widget
                    onclick: this.onclick
                }),
                m__default["default"](".w-100.flex-auto.flex.flex-column-reverse.items-center.pointer.ma3", {
                    classes: theme().uiPdfBottomBarText,
                    onclick: this.onclick
                }, 
                // Form action text
                m__default["default"]("p.mv2.f5.tc", viewController.syncController.formAction.label))
            ];
        }
    }

    class PdfFooterComplete {
        view({ attrs: { viewController } }) {
            var _a, _b;
            // Completed
            return m__default["default"]("p.self-center.mv0.fw3.tc.ui-builder-accent", [
                // Document complete notification
                ((_a = viewController.syncController.formAction) === null || _a === void 0 ? void 0 : _a.completeIcon) && m__default["default"]("i.mr2", {
                    class: viewController.syncController.formAction.completeIcon
                }),
                (_b = viewController.syncController.formAction) === null || _b === void 0 ? void 0 : _b.complete
            ]);
        }
    }

    class PdfFooterCustom {
        view({ attrs: { viewController } }) {
            return [
                viewController.customAction && buildComponent(viewController.customAction.comp)
            ];
        }
    }
    ...
    class Carousel {
        view({ children }) {
            return m__default["default"]('.flex.flex-row', {
                class: theme().uiCarouselWrapper,
            }, children);
        }
    }

    class FieldListSelect extends Themable {
        constructor() {
            super(...arguments);
            this.open = stream__default["default"](false);
            this.maxCount = stream__default["default"]();
            this.selectedCount = stream__default["default"]();
            this.lastActiveField = stream__default["default"]();
        }
        oninit({ attrs: { applyTheme, fieldSet, maxSelection } }) {
            this.createTheme(applyTheme);
            this.maxCount(maxSelection || fieldSet().length);
            this.checkLogic(fieldSet);
        }
        onbeforeupdate({ attrs: { applyTheme } }) {
            this.updateTheme(applyTheme);
        }
        checkLogic(fieldSet) {
            this.selectedCount(lodash__default["default"].reduce(fieldSet(), (acc, cur) => {
                if (this.checkboxActive(cur)) {
                    acc++;
                }
                return acc;
            }, 0));
            if (this.maxCount() === 1) {
                let nextActiveField = '';
                lodash__default["default"].forEach(fieldSet(), (field) => {
                    const { key } = field;
                    if (field.value) {
                        if (this.lastActiveField() === key) {
                            field.value(false);
                        }
                        if (field.value()) {
                            nextActiveField = field.key;
                        }
                    }
                });
                this.lastActiveField(nextActiveField);
            }
        }
        checkboxActive(field) {
            var _a;
            if (!field.value)
                return false;
            if (((_a = field.input) === null || _a === void 0 ? void 0 : _a.type) === "radio") {
                return field.value() === field.input.value;
            }
        ...
        isDisabled(value) {
            if (this.maxCount() === 1)
                return false;
            return this.selectedCount() >= this.maxCount() && !value ? true : false;
        }
        unifyState(fieldSet, formState) {
            return fieldSet().map((field) => {
                return formState.getState(field.data);
            }).reduce((result, currVal) => {
                return Object.assign(result, currVal);
            });
        }
        assembleCheckBox(fieldSet, formState) {
            return lodash__default["default"].map(fieldSet(), (field, i) => {
                // TODO implement state changes
                return assembleFormField(field, lodash__default["default"].merge({
                    uiClass: {
                        wrapper: theme().uiFieldListSelectFieldWrapper,
                        inputWrapper: theme().uiFieldListSelectFieldInputWrapper,
                    },
                    disabled: this.isDisabled(this.checkboxActive(field)),
                    instant: true,
                    id: i === 0 ? 'overlay-assist' : `overlay-assist-${i}`
                }, this.unifyState(fieldSet, formState)));
            });
        }
        view({ attrs: { fieldSet, label, formState } }) {
            return m__default["default"]('.pointer.select-wrapper', {
                class: joinClasses([
                    this.overwrite.uiFieldListSelectWrapper,
                    theme().uiFieldListSelectWrapper,
                ]),
                onclick: () => {
                    this.open(!this.open());
                }
            ...
            m__default["default"]('div', {
                class: joinClasses([
                    this.overwrite.uiFieldListSelectInnerWrapper,
                    theme().uiFieldListSelectInnerWrapper,
                ]),
            }, label ? m__default["default"]("span", label) : m__default["default"]("span", fieldSet()[0].fieldSet), m__default["default"]("i", {
                class: joinClasses([
                    this.overwrite.uiFieldListSelectIcon,
                    theme().uiFieldListSelectIcon
                ]),
                style: {
                    transform: this.open() ? "rotate(180deg)" : "rotate(0deg)"
                },
            })), m__default["default"]('.absolute', {
                style: {
                    pointerEvents: this.open() ? 'auto' : 'none',
                    opacity: this.open() ? '1' : '0',
                },
                class: joinClasses([
                    this.overwrite.uiFieldListWrapper,
                    theme().uiFieldListWrapper
                ]),
                onchange: () => this.checkLogic(fieldSet)
            }, [
                this.assembleCheckBox(fieldSet, formState),
            ]));
        }
    }

    class PdfFooterInput {
        oncreate({ attrs: { viewController } }) {
            var _a;
            const active = viewController.getActiveField();
            this.isSignWidget = ((_a = active.input) === null || _a === void 0 ? void 0 : _a.type) === "sign";
            if (this.isSignWidget && active.files) {
                this.onSignChange = active.files.map((fileList) => {
                    if (fileList.length) {
                        viewController.setNextWidget();
                        viewController.formController.removeFromList(active.key);
                    }
                });
            }
            if (active.value) {
                this.onValueChange = active.value.map(() => {
                    viewController.formController.filterEditList();
                });
            }
        }
        onremove() {
            var _a, _b;
            (_a = this.onSignChange) === null || _a === void 0 ? void 0 : _a.end();
            (_b = this.onValueChange) === null || _b === void 0 ? void 0 : _b.end();
        }
        view({ attrs: { viewController, active } }) {
            var _a, _b, _c;
            const { activeFieldUsers, currentUser, activeUsers } = viewController.syncController;
            // Get user occupying field
            const activeUserId = activeFieldUsers.get(active.key);
            const occupied = activeUserId !== currentUser.id;
            const activeUser = pickByProperty(activeUsers, { licensee: activeUserId });
            const typingUser = activeUser ? getInitials(activeUser) : null;
            const formState = viewController.formController.formState;
            const currentInput = viewController.getActiveField();
            const fieldConfig = (_a = currentInput.input) === null || _a === void 0 ? void 0 : _a.config;
            return [
                viewController.activeFieldInSet()
                    // "Field set" widget view
                    ? m__default["default"]('div', { style: { margin: '0px 2px' } }, m__default["default"](FieldListSelect, {
                        disabled: occupied,
                        fieldSet: viewController.getActiveFieldSet(),
                        formState,
                        maxSelection: viewController.formActionSetSelectionCount(currentInput)
                    }))
                    // "Standard" widget view
                    : assembleFormField(currentInput, lodash__default["default"].merge({
                        id: "overlay-assist",
                        uiClass: {
                            wrapper: theme().uiPdfFooterWrapper,
                            inputWrapper: theme().uiPdfFooterInputWrapper
                        },
                        config: Object.assign(Object.assign({}, fieldConfig), { stampBtnClass: (_b = fieldConfig === null || fieldConfig === void 0 ? void 0 : fieldConfig.stampBtnClass) !== null && _b !== void 0 ? _b : theme().uiPdfFooterSignButton }),
                        disabled: occupied,
                        instant: true,
                        layout: "floatAlways" /* LayoutType.floatAlways */,
                        label: this.isSignWidget ? '' : (_c = currentInput.input) === null || _c === void 0 ? void 0 : _c.label
                    }, formState.getState(currentInput.data))),
                // Occupied and widget is used by a user
                occupied && activeUserId != null && m__default["default"](".flex.justify-center.f7", {
                    style: {
                        height: '0',
                        color: viewController.syncController.getUserColorFromId(activeUserId),
                        transform: "translateY(-1.7rem)"
                    }
                }, m__default["default"](UserTyping, {
                    dotStyle: {
                        background: viewController.syncController.getUserColorFromId(activeUserId),
                    }
                }, m__default["default"]("span.ml1", `${typingUser} is typing...`)))
            ];
        }
    }
    ...
    */
    ...
    var Draggable = /*#__PURE__*/function (_EventDispatcher) {
      ...
      function Draggable(target, vars) {
        ...
        gsap$2.getProperty(target, "x"); // to ensure that transforms are instantiated.
        ...
        syncXY = function syncXY(skipOnUpdate, skipSnap) {
          ...
          gsCache.uncache && gsap$2.getProperty(target, "x"); // trigger a re-cache
        ...
        onPress = function onPress(e, force) {
          ...
          self.isThrowing = false;
          gsap$2.killTweensOf(scrollProxy || target, killProps, true); //in case the user tries to drag it before the last tween is done.
    ...
    */
    ...
    class PdfFormGroupInfoItem {
        onupdate({ attrs: { active }, dom }) {
            if (active) {
                const parent = dom.parentElement;
                const parentWidth = parent.getBoundingClientRect().width || 0;
                const domWidth = dom.getBoundingClientRect().width;
                gsap__default["default"].to(parent, {
                    scrollTo: {
                        x: dom,
                        offsetX: parentWidth * 0.5 - domWidth * 0.5
                    }, ease: "expo",
                    duration: 0.5
                });
            }
        }
        view({ attrs: { active, progress, title, onclick } }) {
            const complete = progress === 100;
            return m__default["default"]('div', {
                class: theme().uiPdfFormGroupInfoItemWrapper,
                onclick
            }, [
                m__default["default"]('div', {
                    class: theme().uiPdfFormGroupInfoItemInner,
                    style: {
                        opacity: active ? 1 : 0.4,
                        color: !complete ? "var(--ui-builder-text)" : 'green'
                    }
                }, title),
                m__default["default"](ProgressBar, {
                    active,
                    progress
                })
            ]);
    ...
    class PdfFormGroupController {
        view({ attrs: { viewController, onSelect } }) {
            const groups = viewController.formController.fieldGroups;
            const activeGroup = viewController.formController.activeGroup;
            return groups && Object.entries(groups)
                .filter(([key]) => key !== 'ungrouped')
                .map(([groupId, fields]) => m__default["default"](PdfFormGroupInfoItem, {
                active: activeGroup() === groupId,
                progress: getPercentage(getRequired(fields, viewController)),
                title: groupId,
    ...
    class PdfFooterEdit {
        oncreate({ attrs: { viewController } }) {
            window.addEventListener("keydown", viewController.tabControls);
        }
        onremove({ attrs: { viewController } }) {
            window.removeEventListener("keydown", viewController.tabControls);
        }
        view({ attrs: { viewController } }) {
            var _a, _b;
            const showGroupController = Object.keys(viewController.formController.fieldGroups).length > 1;
            const active = viewController.formController.activeField();
            const showNavBtn = viewController.formController.editList.length > 1;
            const submitConfig = viewController.formAction.submit;
            const noAction = viewController.formController.actionCount() !== 0;
            return m__default["default"](".flex.center", {
                style: {
                    maxHeight: "100%"
                }
            }, m__default["default"](".flex.flex-column", {
                style: {
                    width: 'calc(100vw - 0.5rem)'
                }
            }, showGroupController && m__default["default"](Carousel, m__default["default"](PdfFormGroupController, {
                viewController,
                onSelect: lodash__default["default"].bind(viewController.setGroup, viewController)
            })), active ? [
                ((_a = active.data) === null || _a === void 0 ? void 0 : _a.preText) && m__default["default"]("p", {
                    class: theme().pdfFooterEditPreText,
                }, active.data.preText),
                m__default["default"](".flex.items-end.center", 
                // Prev Button
                showNavBtn && m__default["default"]('i.grow.pointer.fal.fa-arrow-circle-left', {
                    class: theme().pdfFooterEditNavigate,
                    style: {
                        opacity: viewController.firstActive() ? 0.4 : 1,
                        pointerEvents: viewController.firstActive() ? "none" : "auto",
                    },
                    onclick: lodash__default["default"].bind(viewController.setNextWidget, viewController, -1)
                }), m__default["default"]("form", {
                    class: theme().uiPdfFooterFormWrapper,
                    onsubmit: () => {
                        viewController.setNextWidget();
                        return false;
                    }
                }, m__default["default"](PdfFooterInput, {
                    // Key widget to prevent elements being recycled
                    key: active.key,
                    viewController,
                    active
                })), 
                // Next Button
                showNavBtn && m__default["default"]('i.grow.pointer.fal.fa-arrow-circle-right', {
                    class: theme().pdfFooterEditNavigate,
                    onclick: lodash__default["default"].bind(viewController.setNextWidget, viewController, 1)
                })),
                ((_b = active.data) === null || _b === void 0 ? void 0 : _b.postText) && m__default["default"]("p", {
                    class: theme().pdfFooterEditPostText,
                }, active.data.postText),
            ]
                // Submit button (no active widget selected)
                : [
                    m__default["default"](".flex.flex-column.items-center", [
                        (submitConfig === null || submitConfig === void 0 ? void 0 : submitConfig.preText) && m__default["default"]('.tc', submitConfig.preText),
                        noAction && (submitConfig === null || submitConfig === void 0 ? void 0 : submitConfig.validationMessage) && m__default["default"]('.tc', submitConfig === null || submitConfig === void 0 ? void 0 : submitConfig.validationMessage),
                        ((!viewController.submitted() && viewController.formController.isSingleSignForm)
                            || !viewController.formController.isSingleSignForm) && m__default["default"](uiWidgets.Button, {
                            classes: "mv2",
                            label: "Submit",
                            onclick: lodash__default["default"].bind(viewController.submit, viewController),
                            disabled: noAction
                        }),
                        viewController.syncController.saveSupported && m__default["default"]("p", {
                            class: joinClasses([theme().uiPdfSaveAndExitButton, "underline pointer"]),
                            onclick: () => viewController.close()
                        }, "Save and Exit"),
                        (submitConfig === null || submitConfig === void 0 ? void 0 : submitConfig.postText) && m__default["default"]('.tc', submitConfig === null || submitConfig === void 0 ? void 0 : submitConfig.postText)
                    ])
                ]));
        }
    }
    ...
    const stateComponentMap = {
        ["view" /* PdfState.view */]: null,
        ["complete" /* PdfState.complete */]: PdfFooterComplete,
        ["call" /* PdfState.call */]: PdfFooterCall,
        ["edit" /* PdfState.edit */]: PdfFooterEdit,
        ["custom" /* PdfState.custom */]: PdfFooterCustom
    };
    class PdfFooter {
        oncreate() {
            ...
            if (input) {
                ...
                input.addEventListener("click", this.onClickListener);
            }
        }
        onremove() {
            const input = document.getElementById("hidden-input");
            if (input && this.onClickListener) {
                input.removeEventListener("click", this.onClickListener);
            }
        }
        // Focus on input widget after redrawing next input
        onupdate({ dom, attrs: { viewController } }) {
            if (viewController.focusInput) {
                viewController.focusInput = false;
                requestAnimationFrame(() => { var _a; return (_a = document.getElementById("overlay-assist")) === null || _a === void 0 ? void 0 : _a.focus(); });
            }
            const fabWrapper = document.querySelector("[class^=uiFabWrapper]");
            if (fabWrapper) {
                fabWrapper.style.bottom = `${dom.clientHeight + 16}px`;
            }
        }
        view({ attrs: { viewController } }) {
            const { viewState } = viewController;
            const { formAction } = viewController.syncController;
            const enableAttachments = formAction === null || formAction === void 0 ? void 0 : formAction.enableAttachments;
            const footerComp = stateComponentMap[viewState()];
            return viewState() !== "view" /* PdfState.view */ && m__default["default"]("div", m__default["default"]('div', {
                class: theme().uiFabWrapper
            }, [
                viewController.showRequestPartysFab && m__default["default"](RequestPartyFab, { viewController }),
                enableAttachments && m__default["default"](FileFab, { fileList: viewController.formController.attachments })
            ]), m__default["default"]("div.relative.flex.flex-column.justify-around.items-center.pa1.shadow-2.flex-shrink-0.touch-action-none", {
                class: theme().uiPdfBottomBar
            }, footerComp && m__default["default"](footerComp, { viewController }), 
            // Help iOS keep keyboard visible when changing active widget
            m__default["default"]("input#hidden-input.clip.bottom-0", { style: { visibility: "hidden" } })));
        }
    }

    class PdfStatus {
        view({ attrs: { viewController } }) {
            const isConnected = viewController.syncController.mutltiUser
                ? viewController.syncController.isConnected()
                : true;
            const iconClass = joinClasses([
                viewController.syncController.saving() && isConnected ? "" : theme().uiPdfStatusIcon,
                viewController.syncController.saving() && isConnected ? "fas fa-spinner fa-spin fa-fw mt1" : "",
                !viewController.syncController.saving() && isConnected ? "fal fa-check fa-fw" : "",
                !isConnected ? "fa-inverse fas fa-times" : ""
            ]);
            return m__default["default"]("span.fa-stack", {
                title: 'Sync',
                class: "mr2",
                style: {
                    color: "#969696",
                ...
                onclick: () => viewController.syncController.saveState()
            }, 
            // TODO consider adding some helpful tool tip (refresh page e.g.)
            m__default["default"]('i.mr2.fa-stack-2x', { class: isConnected ? 'fal fa-cloud' : "fas fa-cloud" }), m__default["default"]('i.mr2.fa-stack-1x', { class: iconClass, style: { transform: "translateY(2px)" } }));
        }
    }

    function pdfItem(path) {
        return {
            _id: "unset",
            modelId: "unset",
            tags: [],
    ...
    function getDocUrl(item) {
        return item.files[0] && item.files[0].remoteUrl;
    }
    class PdfViewer {
        constructor() {
            this.statusText = "";
        }
        oninit({ attrs: { item, onSubmit, onClose, disableBackNavigation, useItemOverlays } }) {
            this.documentUrl = getDocUrl(item);
            this.createViewHelper(item, onSubmit, onClose, disableBackNavigation, useItemOverlays);
        }
        onbeforeupdate({ attrs: { item, onSubmit, onClose, disableBackNavigation, useItemOverlays } }) {
            const newUrl = getDocUrl(item);
        ...
        createViewHelper(item, onSubmit, onClose, disableBackNavigation, useItemOverlays) {
            if (this.documentUrl) {
                const filePath = this.documentUrl;
                this.destroyViewer();
                this.destroyViewController();
                this.setStatusText("Loading...");
                m__default["default"].redraw();
                pdfViewerFactory(this.documentUrl)
                    .then((viewer) => {
                    this.viewer = viewer;
                    viewer.parseCustomMetadata().then((viewer) => {
                        this.viewController = new ViewController(item, viewer.overlayList, viewer.action, filePath, onSubmit, onClose, !disableBackNavigation, viewer.metadata, viewer.custom, useItemOverlays);
                        m__default["default"].redraw();
                    });
                })
                    .catch(lodash__default["default"].flow([
                    lodash__default["default"].partialRight(humaniseErrorCode, filePath),
                    lodash__default["default"].bind(this.setStatusText, this)
                ]))
                    .then(m__default["default"].redraw);
            }
            else {
                this.setStatusText("No File Present");
            }
        }
        ...
        destroyViewController() {
            var _a;
            (_a = this.viewController) === null || _a === void 0 ? void 0 : _a.destroy();
            this.viewController = undefined;
        }
        view() {
            var _a, _b, _c;
            const overallRequiredFields = this.viewController
                ? getRequired(this.viewController.formController.editList, this.viewController)
                : [];
            return m__default["default"](".flex.flex-column.flex-auto.h-100", {
                class: theme().uiPdfViewerWrapper
            }, [
                m__default["default"](PanelHeader, {
                    classes: joinClasses(["h2.5", theme().uiPdfTopBar])
                }, [
                    // Use ui-widgets sign font
                    m__default["default"]("span.clip", { style: { "font-family": ((_a = application().uiWidgets) === null || _a === void 0 ? void 0 : _a.signFont) || "Caveat" } }, "sign font"),
                    ((_b = this.viewController) === null || _b === void 0 ? void 0 : _b.syncController.mutltiUser) && m__default["default"](ActiveUsers, {
                        viewController: this.viewController
                    }),
                    noreturn()
                        ? m__default["default"]("div")
                        : m__default["default"](uiWidgets.NavButton, {
                            icon: "fal fa-times nl2",
                            title: "Back",
                            classes: joinClasses(["f3 dim", theme().uiNavButton, theme().uiPdfBackButton]),
                            onclick: () => { var _a; return (_a = this.viewController) === null || _a === void 0 ? void 0 : _a.close(); }
                        }),
                    m__default["default"]("div.flex.justify-around", this.viewController && [
                        m__default["default"](uiWidgets.NavButton, {
                            icon: "fal fa-search-plus fa-fw",
                            title: "Zoom In",
                            classes: joinClasses(["f3 dim", theme().uiNavButton, theme().uiPdfZoomInButton]),
                            disabled: this.viewController.zoom() >= 2 /* ZoomAmount.Max */,
                            onclick: lodash__default["default"].bind(this.viewController.adjustZoom, this.viewController, 0.5 /* ZoomAmount.Increment */)
                        }),
                        m__default["default"](uiWidgets.NavButton, {
                            icon: "fal fa-search-minus fa-fw",
                            title: "Zoom Out",
                            classes: joinClasses(["f3 dim", theme().uiNavButton, theme().uiPdfZoomOutButton]),
                            disabled: this.viewController.zoom() <= -2 /* ZoomAmount.Min */,
                            onclick: lodash__default["default"].bind(this.viewController.adjustZoom, this.viewController, -0.5 /* ZoomAmount.Decrement */)
                        })
                    ]),
                    ((_c = this.viewController) === null || _c === void 0 ? void 0 : _c.syncController.syncSupported) && m__default["default"](`.absolute.${theme().uiPdfSyncButtonWrapper}`, {
                        style: {
                            right: '3rem'
                        }
                    }, m__default["default"](PdfStatus, {
                        viewController: this.viewController
                    })),
                    this.documentUrl && m__default["default"](uiWidgets.NavLink, {
                        icon: "fal fa-arrow-to-bottom",
                        title: "Download",
                        classes: joinClasses(["f3 dim", theme().uiNavButton, theme().uiPdfDownloadButton]),
                        href: this.documentUrl
                    })
                ]),
                // Progress Bar
                overallRequiredFields.length > 0 && m__default["default"](PanelSubheader, m__default["default"](ProgressBar, {
                    active: true,
                    progress: getPercentage(overallRequiredFields)
                })),
                // PDF Viewer
                this.viewer && this.viewController
                    ? [
                        ...
                        m__default["default"](OverlaySpinner, {
                            active: this.viewController.submitted(),
                            text: "Submitting...",
                        })
                    ]
                    // Status indicator (loading/error)
                    : m__default["default"](".flex-auto.self-stretch", {
                        class: joinClasses([theme().uiPdfPanel])
                    }, m__default["default"]("span.ma3.db.truncate", {
                        class: joinClasses([theme().uiPdfSubheader])
                    }, this.statusText)),
                // Bottom toolbar area
                this.viewController && m__default["default"](PdfFooter, {
                    viewController: this.viewController
                })
            ]);
        }
    ...
    class SlideOut {
        ...
        view({ attrs: { title, width = "30%", buttonLeft, style = {} }, children }) {
            return m__default["default"](".flex.flex-column", {
                style: Object.assign({ background: "white", transition: "width 0.5s ease-in-out", boxShadow: "0 8px 15px rgb(0 0 0 / 20%)", width: this.collapsed ? '3.5rem' : width }, style)
            }, [
                // Header
                m__default["default"](".flex.flex-column", {
                    style: {
                    ...
                    class: buttonLeft ? "ma3 mr-auto " : "ma3 ml-auto",
                    style: {
                        color: "white",
                        height: '2rem',
                        width: "2rem",
                ...
                }, m__default["default"]('i.fas.fa-ellipsis-v.center', {
                    style: {
                        transition: "transform 0.2s ease-in-out",
                        transform: `rotate(${this.collapsed ? 90 : 0}deg)`
                    }
                })), title && m__default["default"]("span.white.f3", {
                    class: buttonLeft ? "ma3 mr-auto" : "ma3 ml-auto",
                    style: {
                ...
                // Content
                m__default["default"](".flex.flex-column.flex-auto.overflow-y-scroll", {
                    style: {
                        transition: "opacity 0.4s ease-in-out",
                        opacity: this.collapsed ? "0" : "1"
                    }
    ...
    const mutableModel = stream__default["default"]();
    ...
    class SingleGrid {
        oncreate({ dom, attrs }) {
            lodash__default["default"].defaults(attrs.gridOptions, SingleGrid.defaultGridOpts, {
                rowClass: theme().uiAgGridRow
            });
            new agGridCommunity.Grid(dom, attrs.gridOptions);
            this.postGridOptions(attrs);
            // Load SingleGrid dataset
            this.showLoading(attrs.gridOptions);
            attrs.load({ offset: 0, limit: -1 }).then((rowData) => {
                var _a;
                this.hideLoading(attrs.gridOptions);
                (_a = attrs.gridOptions.api) === null || _a === void 0 ? void 0 : _a.setRowData(rowData);
            });
        }
        postGridOptions({ gridOptions, columnAutoResize, hideColumns }) {
            var _a;
            if (columnAutoResize) {
                gridOptions.onGridSizeChanged = () => { var _a; return (_a = gridOptions.api) === null || _a === void 0 ? void 0 : _a.sizeColumnsToFit(); };
                (_a = gridOptions.api) === null || _a === void 0 ? void 0 : _a.sizeColumnsToFit();
            }
            const colApi = gridOptions.columnApi;
            if (hideColumns && colApi) {
                this.onResize = screenSize.map(({ small, medium, large }) => {
                    var _a;
                    const allColIds = lodash__default["default"].concat(hideColumns.small, hideColumns.medium, hideColumns.large);
                    colApi.setColumnsVisible(allColIds, true);
                    if (small) {
                        colApi.setColumnsVisible(hideColumns.small, false);
                    }
                    if (medium) {
                        colApi.setColumnsVisible(hideColumns.medium, false);
                    }
                    if (large) {
                        colApi.setColumnsVisible(hideColumns.large, false);
                    }
                    (_a = gridOptions.api) === null || _a === void 0 ? void 0 : _a.sizeColumnsToFit();
                });
            }
        }
        onremove({ attrs: { gridOptions } }) {
            var _a, _b;
            (_a = this.onResize) === null || _a === void 0 ? void 0 : _a.end(true);
            (_b = gridOptions.api) === null || _b === void 0 ? void 0 : _b.destroy();
        }
        view({ attrs: { gridClass } }) {
            return m__default["default"](".ag-theme-alpine", {
                class: joinClasses([theme().uiAgGrid, gridClass])
            });
        }
        showLoading(gridOptions) {
            var _a;
            (_a = gridOptions.api) === null || _a === void 0 ? void 0 : _a.showLoadingOverlay();
        }
        hideLoading(gridOptions) {
            var _a;
            (_a = gridOptions.api) === null || _a === void 0 ? void 0 : _a.hideOverlay();
        }
    }
    SingleGrid.defaultGridOpts = {
        suppressCellSelection: true,
        tooltipShowDelay: 50
    };

    function getDataSource(load, showLoading, hideLoading) {
        return {
            getRows: ({ startRow, endRow, successCallback, failCallback }) => {
                const limit = endRow - startRow;
                showLoading();
                // Load extra row to test if paging has reached end
                load({ offset: startRow, limit: limit + 1 })
                    .then(function (data) {
                    if (data.length > limit) {
                        // More rows to request
                        successCallback(data.slice(0, limit));
                    }
                    else {
                        // All rows loaded
    ...
    class PagingGrid extends SingleGrid {
        oncreate({ dom, attrs }) {
            lodash__default["default"].defaults(attrs.gridOptions, PagingGrid.defaultGridOpts, {
                rowClass: theme().uiAgGridRow
            });
            attrs.gridOptions.datasource = getDataSource(attrs.load, lodash__default["default"].partial(this.showLoading, attrs.gridOptions), lodash__default["default"].partial(this.hideLoading, attrs.gridOptions));
            new agGridCommunity.Grid(dom, attrs.gridOptions);
            this.postGridOptions(attrs);
        }
    }
    PagingGrid.defaultGridOpts = {
        suppressCellSelection: true,
        tooltipShowDelay: 50,
    ...
    const nullModel = {
        ...
        readonly: true,
        label: "Unknown",
        icon: "fal fa-undefined",
        action: "view" /* ActionType.view */,
        fields: [],
        listType: "basic" /* ListType.basic */,
        search: [],
        heading: [],
        subheading: []
    };
    const categoryModel = {
        ...
        label: "Category",
        icon: "fal fa-filter",
        action: "list" /* ActionType.list */,
        fields: [],
        listType: "category" /* ListType.category */,
        search: [],
        heading: [],
        subheading: []
    };
    ...
    function createCategories(itemList, driven, parent) {
        const { _id, readonly, icon, type, tags: categoryTags, contentAll, countContent, hideEmpty, search, sort, group, gridConf, creationContext: ctx, child } = driven;
        const driverId = parent ? `${parent._id}-${_id}` : _id;
        // Scope list to content tags
        const content = parent ? parent.content : driven.content;
        const contentItems = itemsForCategory(itemList, content, contentAll);
        // Generate categories for driver creationContext
        return lodash__default["default"](contentItems)
            .map(({ tags }) => tags ? pickByProperty(tags, { ctx }) : null)
            .compact()
            .map(({ val }) => val)
            .uniq()
            .map((val) => {
            const ctxTag = { ctx, val };
            const category = {
                _id: `${driverId}-${val}`,
                modelId: "category",
                readonly,
                ...
                countContent: countContent
                    ...
                    : undefined,
                tags: categoryTags,
                hideEmpty, search, sort, group, gridConf
            };
            if (child) {
                // Update parent and child tagging to create hierarchy
                const parentCategory = lodash__default["default"].extend({}, category, {
                    content: lodash__default["default"].concat(child.tags, ctxTag)
                });
                const childCategory = lodash__default["default"].extend({}, child, {
                    tags: parentCategory.content,
                    // Enforce "contentAll" tag matching for child folders
                    contentAll: true
                });
                // Generate next level of categories
                const childCategories = createCategories(contentItems, childCategory, category);
    ...
    function fullFileName(name, filetype) {
        const [fname, ext] = uiWidgets.fileNameExtSplit(name);
        if (ext) {
            return name;
        }
        else {
    ...
    function xformAsset({ id, uuid, origin, href, url = href, date_deposited: date, name, size, viewed, filetype: type, category, subcategory, reference, actionsjson: actions, metadata }) {
        const item = {
            _id: uuid,
            modelId: "documents",
            tags: [{
            ...
            actions: actions ? strToJSON(actions) : [],
            metadata: metadata ? transformMetadata(metadata) : {}
        };
        return item;
    }

    function scopeList(sourceList, filterScope) {
        return lodash__default["default"].filter(sourceList, ({ src: { scope } }) => scope === filterScope);
    }
    function getUserSource(sourceList) {
        const filtered = scopeList(sourceList, "user" /* DataScope.User */);
        return filtered.length ? filtered[0] : null;
    }
    function buildSources(sourceList, type) {
        return lodash__default["default"].map(filterByProperty(sourceList, { type }), (src) => new DataSource(src));
    }
    class Items {
        constructor(config = {}) {
            this.remotePath = "./";
            ...
            const sourceList = this.config.sourceList || [];
            // Models
            this.modelSources = buildSources(sourceList, "model" /* DataType.Model */);
            // Combine loaded models with special category model
            this.modelList = stream__default["default"].merge(lodash__default["default"].map(this.modelSources, (src) => src.stream))
                .map((modelLists) => lodash__default["default"].concat([categoryModel], lodash__default["default"].flatten(modelLists)));
            this.modelRecord = this.modelList.map((list) => lodash__default["default"].keyBy(list, "_id"));
            // Assets
            this.assetSources = buildSources(sourceList, "asset" /* DataType.Asset */);
            // Transform assets into "typical" items
            this.assetList = stream__default["default"].merge(lodash__default["default"].map(this.assetSources, (src) => src.stream))
                .map((assetLists) => lodash__default["default"].map(lodash__default["default"].flatten(assetLists), xformAsset));
            // Items
            this.itemSources = buildSources(sourceList, "item" /* DataType.Item */);
            // Patch item files lacking remoteUrl property
            this.itemList = stream__default["default"]
                .merge(lodash__default["default"].map(this.itemSources, (src) => src.stream))
                .map((itemLists) => lodash__default["default"]
                .map(lodash__default["default"].flatten(itemLists), lodash__default["default"].bind(this.patchRemoteUrl, this)));
            // Templates
            this.teamplateSources = buildSources(sourceList, "template" /* DataType.Template */);
            this.templateMap = stream__default["default"]
                .merge(lodash__default["default"].map(this.teamplateSources, (src) => src.stream))
                .map((templateLists) => lodash__default["default"].chain(templateLists)
                .flatten()
                .keyBy(({ templateKey }) => templateKey)
                .value());
            // Template data records
            this.templateDataSources = buildSources(sourceList, "templateData" /* DataType.TemplateData */);
            // Build items from template data
            this.buildList = stream__default["default"]
                .merge(lodash__default["default"].map(this.templateDataSources, (src) => stream__default["default"]
                .lift((objects, templateMap) => {
                // Get template match configuration from source config
                const matchObj = "matchTemplate" in src.src ? src.src.matchTemplate : { key: "type" };
                return buildTemplates(objects, templateMap, (object, templates) => {
                    const templateKey = object[matchObj.key];
            ...
            }, src.stream, this.templateMap)))
                .map(lodash__default["default"].flatten);
            lodash__default["default"].bind(this.patchRemoteUrl, this);
            // Combine items and assets
            this.dataList = stream__default["default"].lift((assets, items, build) => {
                const list = lodash__default["default"].concat(assets, items, build);
                // Add default "root" category if required
                const root = pickById(list, "root");
                if (!root) {
                    list.push(homeCategory);
                }
                // "Unpack" data-driven objects into their respective categories
                const drivers = removeByProperty(list, { modelId: "driven" });
                return lodash__default["default"].concat(list, lodash__default["default"](drivers)
                    .map((driver) => createCategories(list, driver))
                    .flatten()
                    .value());
            }, this.assetList, this.itemList, this.buildList);
        }
        loaded() {
        ...
        loadApplication() {
            return this.loadScope("application" /* DataScope.Application */);
        }
        /** Load all User Scoped DataSources */
        loadUser() {
            return this.loadScope("user" /* DataScope.User */);
        }
        loadScope(scope) {
            return Promise.all([
                Promise.all(lodash__default["default"].map(scopeList(this.modelSources, scope), (src) => src.load())),
                Promise.all(lodash__default["default"].map(scopeList(this.assetSources, scope), (src) => src.load())),
                Promise.all(lodash__default["default"].map(scopeList(this.itemSources, scope), (src) => src.load())),
                Promise.all(lodash__default["default"].map(scopeList(this.teamplateSources, scope), (src) => src.load())),
                Promise.all(lodash__default["default"].map(scopeList(this.templateDataSources, scope), (src) => src.load()))
            ]);
        }
        /** Load assets from a specific sjr and merge with existing loaded assets */
        updateAssets(params) {
            const userAssets = getUserSource(this.assetSources);
            if (userAssets && userAssets.src.path) {
                return sdxRequest(userAssets.src.path, { params })
                    .then((list) => this.addAssets(list));
            }
        ...
        addAssets(assets) {
            const userAssets = getUserSource(this.assetSources);
            if (userAssets) {
                const addList = userAssets.stream();
                let addedCount = 0;
                lodash__default["default"].each(assets, (asset) => {
                    if (pickByProperty(addList, { uuid: asset.uuid }) === undefined) {
                        ++addedCount;
                        addList.push(asset);
                    }
                });
                userAssets.stream(addList);
                return addedCount;
            }
            return 0;
        }
        viewAsset({ uuid }) {
            const userAssets = getUserSource(this.assetSources);
            if (userAssets) {
                const updateList = userAssets.stream();
                // Replace asset with version marked as viewed
                const assets = removeByProperty(updateList, { uuid });
                if (assets.length) {
                    const viewed = assets[0];
                    viewed.viewed = true;
        ...
        upsertUserItem(item) {
            const userItems = getUserSource(this.itemSources);
            if (userItems) {
                const updateList = userItems.stream();
        ...
        removeUserItem({ _id }) {
            ...
            if (userItems) {
                const updateList = userItems.stream();
                removeByProperty(updateList, { _id });
                userItems.stream(updateList);
                return this.saveUserData(userItems.stream());
            }
            else {
        ...
        saveUserData(list) {
            const updatePath = this.config.itemUpdate;
            if (!updatePath) {
                return Promise.resolve();
            }
            return sdxRequest({
        ...
        patchRemoteUrl(item) {
            ...
            if (dat.files && dat.files.length) {
                return lodash__default["default"].assign({}, dat, {
                    files: lodash__default["default"].map(dat.files, (file) => lodash__default["default"].assign({}, file, {
                        remoteUrl: file.remoteUrl ? file.remoteUrl : `${this.remotePath}${file._id}`
                    }))
                });
            }
            else {
    ...
    class ItemSelect {
        constructor(items) {
            ...
            const modelStream = items.model();
            const itemStream = items.data();
            ...
            this.pickAction = stream__default["default"].lift((item, id) => ("actions" in item ? pickById(item.actions, id) : null) || nullAction, this.pickItem, this.actionId);
            this.readonly = stream__default["default"].lift((model, item) => Boolean(model.readonly || item.readonly), this.pickModel, this.pickItem);
        }
        selectItem(id) {
            this.itemId(id);
            mutableItem(this.pickItem());
        ...
        isCategory() {
            return this.pickModel().category || false;
        }
        selectModel(id) {
            this.modelId(id);
            mutableModel(this.pickModel());
        }
        getModel() {
    ...
    function custom(appConf, { fullScreen, responsiveHeader, responsiveFooter, logout, wrapContent = true, customAttrs, header, central, footer }) {
        return bootstrap(appConf).then(() => {
            m__default["default"].mount(document.getElementById("page"), {
                view: () => m__default["default"](Layout, {
                    ...
                    responsiveFooter,
                    logout,
                    header: header ? m__default["default"](header, customAttrs) : undefined,
                    footer: footer ? m__default["default"](footer, customAttrs) : undefined
                }, wrapContent
                    ? m__default["default"](".flex-auto.overflow-x-hidden.overflow-y-auto.safari-bug", m__default["default"](central, customAttrs))
                    : m__default["default"](central, customAttrs))
            });
        });
    }
    function customRoute(appConf, defaultRoute, routes) {
        return bootstrap(appConf).then(() => {
    ...
    exports.getFileWidget = getFileWidget;
    exports.getFiles = getFiles;
    exports.getISODate = getISODate;
    exports.getPagingGrid = getPagingGrid;
    exports.getProp = getProp;
    exports.getPropStream = getPropStream;
    exports.getPropWidget = getPropWidget;
    exports.getSingleGrid = getSingleGrid;
    ...
    exports.pdfBuilderFactory = pdfBuilderFactory;
